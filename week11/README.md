# Week11
## CRUD 300字
CRUD 是四個英文字的縮寫，分別代表新增（Create）、讀取（Read）、更新（Update）和刪除（Delete）。用大家最熟悉的例子來說，就是手機裡的聯絡人。
當你把朋友的名字和電話輸入手機，就是 Create；打開聯絡人看到所有人的資料，就是 Read；如果朋友換了電話，你把舊號碼改掉，就是 Update；如果你不想再保留某個聯絡人，把它刪掉，就是 Delete。這四個動作看似很普通，但任何 App、網站、系統只要有資料，都一定會用到 CRUD。像購物網站把商品加到購物車、IG 貼文的留言被修改、學校系統查詢成績、甚至刪除雲端硬碟的檔案，這些背後都在做 CRUD，只是界面更漂亮、功能更多而已。CRUD 的重要性就在於它讓資料保持整齊、不混亂，也讓系統能穩定運作。只要懂 CRUD，就能理解大部分的資料流程，也能更快學會資料庫、後端或各種應用程式的運作方式。
